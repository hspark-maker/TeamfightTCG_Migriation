...
자체 코드는 409파일 65,801줄, 공개 타입 469개. `Photon/`, `AmplifyShaderEditor/`, `PurchasedAssets/`, `Plugins/`, `GUIPackCartoon/` 는 서드파티라 여기 없다.
...
- 저장소: `Save/1.Repository/IRepository` · `JsonFileRepository` · `PlayerPrefsRepository`
- 도메인: `Save/2.Domain/UserSaveData` (루트) 아래 `CurrencySaveData` · `CollectionSaveData` · `OwnershipSaveData` · `DeckSaveData` · `CardGrowthSaveData` · `KeywordGrowthSaveData` · `RankSaveData` · `AlbumRewardSaveData` · `TutorialSaveData`
- 매니저: `Save/3.Manager/DataSaveManager` (`DataSaveManager.Load` / `DataSaveManager.Save` / `DataSaveManager.Data`) — 각 기능 매니저가 여기로 flush 한다
...
- 카탈로그·소유: `CardCatalog` · `CatalogRow` · `CatalogRows` · `OwnershipManager` · `CollectionProductionManager`
- 테마: `CollectionThemes` · `CollectionTheme` · `CollectionThemeConfig` · `CollectionLayoutConfig`
- 도감: `CardAlbum` · `AlbumPage` · `AlbumTheme` · `CardAlbumConfig` · `AlbumRewardManager` · `AlbumRewardInfo`
- 컬렉션 UI: `UI/Collection/CollectionGalleryController` · `CollectionGridController` · `CollectionTabController` · `CollectionThemeListController` · `CardDetailOverlayView` (최대 UI 파일)
...
- 실행: `OutgameTutorialRunner` · `TriggeredTutorialRunner` · `Tutorial/Steps/TutorialStepExecutor` · `TutorialStepDef` · `OutgameTutorialStepContext`
...
- 데이터·풀: `Utils/DataLibrary` · `ObjectPooler` · `ParticlePooler` · `PooledParticle` · `GameClock` · `LogUtil` · `CameraUtil`
- 사운드: `Audio/SoundManager` · `SoundConfig` · `UIClickSound`
- 디버그: `OutGame/Debug/OutgameDebugOverlay` · `OutgameDebugActions` · `OutGame/Collection/Debug/OwnershipDebugTool` · `Test/VfxDebugWindow`
