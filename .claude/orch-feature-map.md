...
- 이동 실행 `UI/Mission/GuideMissionNavigator.Go` — 덱 편집기 · 모험 맵(`LobbyMatchLauncher.TryOpenAdventureMapAt` → `AdventureMapOverlayView.FocusNode`) · 도감 탭 · 카드 상세. 도착 코치마크의 유일한 발화처
...
- 자율 발화의 유일한 창구는 `UI/Tutorial/GuidanceCoordinator.TryFire` — 무대(연출·커튼·게이트 표시 중)가 바쁘면 버린다(보류 큐 없음, 알림 점 `UI/Tutorial/TutorialAlertDot` 이 다시 부른다). 발화처는 탭 도착(`LobbyTabController`) · 키워드 패널 열림(`KeywordGrowthPanel`) · 모험 맵 열림(`LobbyMatchLauncher`) · 랭크 연출 종료(`LobbyRankEffectDirector`) · 가이드 미션 "이동" 도착(`UI/Mission/GuideMissionNavigator`, 트리거 `GuideCaretakerEnhanceArrived`·`GuideAceEnhanceArrived` — 자연 진입에는 붙지 않는다)
...
- UI: `UI/Tutorial/OutgameTutorialGateUI` · `OutgameTutorialBridge` (브리지 하나가 강제·자율을 같은 코드로 그린다, `LobbyScene` 의 `TutorialSystem`) · `GuidanceCoordinator` · `SynergyIntroduction` (특수 안내, 조정기 아래) · `UI/TutorialOverlayUI` · `UI/TutorialSetupUI` · `UI/TutorialUIStyle` · `UI/Tutorial/TutorialAlertDot` · `UI/Input/TutorialTapCatcher`
- 에디터: `Editor/Tutorial/TutorialAuthoringWindow` (챕터 성격·발화 키·선행 기능 편집) · `TutorialValidator` (자율 챕터 규약 — 해금/잠금·강제 전용 액션·도중 씬 이탈·발화 키 중복은 Error) · `TutorialSequenceState` · `TutorialSequenceEditOps`
