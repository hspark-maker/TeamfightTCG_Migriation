<!-- 기능 지도 생성 블록은 직접 수정하지 말고 node .claude/sync-agents-map.js 를 실행한다. -->
...
- 게임 콘텐츠의 상태는 글자를 늘리기보다 색·아이콘·문양으로 먼저 표현한다. 카드 강화 단계는 `0성/3성` 같은 숫자 텍스트 대신 별의 채움과 색으로 표시한다. 설명 문장은 필요한 곳에만 사용한다. (2026-09-18 사용자 지시)
...
- Addressables 빌드·카탈로그 수정·Firebase 리소스 배포 전 [오류와 재발 방지 기록](docs/AddressablesDeploymentPitfalls.md)을 읽는다. 런타임 경로 토큰을 에디터 경로로 해석한 뒤 카탈로그에 저장하지 않는다.
- 개발 단계에서는 현재 앱 카탈로그가 참조하지 않는 과거 번들·카탈로그와 미사용 Hosting 버전을 정리해도 된다(2026-09-16 사용자 지시). 현재 사용하는 카탈로그·의존성은 보존하며 원격 파일뿐 아니라 로컬 의존성의 기기 경로도 검증한다. 상세 절차는 [Firebase 리소스 배포](docs/FirebaseResourceHosting.md)를 따른다.
- 카탈로그 의존성은 순서까지 보존한다. 목록을 정렬해 비교하지 않으며, 실제 Provider처럼 첫 번째 번들에서 자산을 읽는 검증을 수행한다.
...
- 초기화 재시도(씬 재로드 없음): 복구 화면의 `UI/Common/LoadingCoverView.Retry` 가 `Core/Initialization/InitializationRunner.RestartGate` 를 부르고 초기화를 다시 돌린다. 재시도 가능 판정은 `Core/GameInitialization.CanRetry`. 실패한 캐시·클라우드 상태를 되돌리는 체인은 아직 없다(코드에 TODO 로 남아 있다) — 같은 실패를 그대로 다시 볼 수 있다
