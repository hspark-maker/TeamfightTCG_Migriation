...
function Disable-Block {
    ...
    Write-Host '다시 Play 해서 정상 초기화되는지 확인하세요 — 규칙이 덜 지워진 상태와 구분됩니다.'
}

if ($Action -ne 'status' -and -not (Test-Admin)) {
    throw '방화벽 규칙 변경에는 관리자 권한이 필요합니다. OfflineInitTest.bat 또는 Unity 메뉴 Tools/Card Battle/오프라인 초기화 검증 으로 실행하세요.'
}
