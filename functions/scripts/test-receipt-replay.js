...
(async () => {
  ...
  // ── 1. 부트 영수증은 두 번 써도 터지지 않는다 ─────────────────────────────
  // 부트 영수증은 create 가 아니라 set 이다. create 였다면 "지갑만 지워지고 영수증이 남은"
  // 계정이 재생성에서 ALREADY_EXISTS 로 영구 실패한다 — 그 계정은 다시는 부트하지 못한다.
  ...
  assert.equal(bootFirst.walletCreated, true, "첫 부트가 지갑을 세운다");
  assert.ok((await receiptIds()).includes(BOOT_RECEIPT_ID), "부트도 영수증을 끊는다");
  ...
  // 아래 영수증 개수 단언을 0 에서 출발시키려고 부트 영수증만 걷어낸다.
  ...
  // 클라가 도메인 거절로 오해하면 부트를 다시 걸지 않고 어긋난 채로 진행한다.
