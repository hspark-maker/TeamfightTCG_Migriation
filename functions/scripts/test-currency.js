...
//
// 여기서 지키는 것은 두 가지다.
//  1) 슬롯 산출이 항상 4키다 — 룰 isValidSave 의 balances.hasOnly 와 갈리면
//     그 계정의 이후 모든 클라 저장이 영구 거부되고 delete: if false 라 복구 경로가 없다.
...
const {readBalances, canAfford, spend, grant, currencySlot} = require("../lib/currency/wallet.js");
...
// 부분 잔액을 넣어도 빠진 키가 0 으로 선다.
assert.deepEqual(currencySlot({Gold: 10}), {balances: {Gold: 10, Diamond: 0, Energy: 0, Shard: 0}});

// ── 차감: 구 buildCurrencySlot 과 동작이 같아야 한다 ─────────────────────────
assert.deepEqual(currencySlot(spend(FULL, "Gold", 120)),
