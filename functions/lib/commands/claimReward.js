 ...
 */
async function loadRankGrades(context) {
    const rows = await (0, packSpecReader_1.readSpecRows)(context.env, "RankGrade");
    let grades = [];
    try {
        grades = (0, payout_1.parseRankGradeRows)(rows);
    }
    catch (error) {
 ...
 */
async function loadAlbumEntries(context) {
    const rows = await (0, packSpecReader_1.readSpecRows)(context.env, "AlbumEntry");
    const entries = (0, completionTable_1.parseAlbumEntryRows)(rows);
    if (entries.length === 0) {
...
exports.claimReward = (0, https_1.onCall)((0, requestMetrics_1.measuredCallable)("claimReward", async (request) => {
    ...
    if (ownerType === "Rank") {
        ...
        if ((0, completionTable_1.parseAlbumScope)(ownerId) === null) {
    ...
    if (!judgement.allow) {
        ...
        reject(judgement.reason, judgement.specEmpty ?
            ...
            `No reward is authored for ${ownerType}/${specOwnerId}.`, { ...context, specOwnerId, specEmpty: judgement.specEmpty, droppedCount: dropped.length });
    }
