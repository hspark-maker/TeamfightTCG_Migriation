...
export const GRADE_KEYS = ["Bronze", "Silver", "Gold", "Platinum", "Diamond"] as const;
 ...
 */
export const FALLBACK_ENTRY_POINTS = [100, 260, 420, 580, 740];

/** RankGrade 표의 한 행. 업로더(SpecFirestoreUploader)가 만드는 필드 그대로다. */
export interface RankGradeRow {
  id: number;
  gradeKey: string;
  entryPoints: number;
}
 ...
 */
export function entryPointsFromRows(rows: RankGradeRow[]): number[] | null {
  const points: number[] = [];
  for (const key of GRADE_KEYS) {
    const row = rows.find((r) => r.gradeKey === key);
    if (row === undefined || !Number.isFinite(row.entryPoints)) return null;
 ...
 * @return {number} 등급 순번
 */
export function gradeOf(entryPoints: number[], points: number): number {
  for (let i = entryPoints.length - 1; i >= 0; i--) {
    if (entryPoints[i] <= points) return i;
  }
  return 0;
}
 ...
 */
export function isRanked(entryPoints: number[], points: number): boolean {
 ...
 * @return {number} 등급 순번
 */
export function parsePoolGrade(value: string): number {
  const index = (GRADE_KEYS as readonly string[]).indexOf(value);
 ...
 */
export function parseRequiredGrade(value: string): number | null {
  const trimmed = value.trim();
  if (trimmed.length === 0) return null;

  const lowered = trimmed.toLowerCase();
  const index = (GRADE_KEYS as readonly string[]).findIndex((k) => k.toLowerCase() === lowered);
  if (index >= 0) return index;

  // Enum.IsDefined 에 걸려 떨어지는 갈래 — 클라는 여기서 경고만 내고 잠금을 풀어 준다.
  const numeric = parseNumericGrade(trimmed);
  if (numeric === null || numeric < 0 || numeric >= GRADE_KEYS.length) return null;
  return numeric;
}
