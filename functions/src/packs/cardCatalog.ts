 ...
 */
export async function loadCatalogIds(env: string): Promise<Set<number>> {
  const isTest = env === "test";
  const rows = await readSpecRows(env, isTest ? "Card_Test" : "Card");

  const ids = new Set<number>();
  for (const row of rows) {
    const id = Number(row.id);
    if (!Number.isInteger(id) || id <= 0) continue;
    if (!isTest && String(row.channel ?? "") !== LIVE_CHANNEL) continue;
