...
/** 추첨 풀의 한 칸. 클라 WeightedCard 의 쌍둥이. */
 ...
 */
export function resolveDropPool(
  rows: DropRow[],
  gradeIndex: number,
  ...
  for (const row of rows) {
    const grade = parsePoolGrade(row.minGrade);
    if (grade > gradeIndex) continue;
  ...
  for (const row of rows) {
    if (parsePoolGrade(row.minGrade) !== best) continue;
    if (!catalogIds.has(row.cardId)) continue;
 ...
 */
function pickWeightedCandidate(pool: WeightedCard[], candidates: number[], roll: RollFn): number {
  let sum = 0;
  for (const index of candidates) sum += effectiveWeight(pool[index].weight);

  // 유효 가중치가 최소 1이라 구조상 도달하지 않는다. 도달했다면 풀이 비었다는 뜻이다.
  if (sum <= 0) return candidates.length - 1;
...
export function drawPack(
  ...
  let count = drawCount;
  if (uniqueDraw && count > pool.length) count = pool.length;

  const candidates: number[] = [];
  for (let i = 0; i < pool.length; i++) candidates.push(i);

  const drawn: DrawnCard[] = [];
  for (let i = 0; i < count; i++) {
    const pick = pickWeightedCandidate(pool, candidates, roll);
    const cardId = pool[candidates[pick]].cardId;
    if (uniqueDraw) candidates.splice(pick, 1);
