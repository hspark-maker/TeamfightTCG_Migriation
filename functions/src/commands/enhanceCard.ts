...
import {canEnhanceSynergyIntroduction, SYNERGY_INTRODUCTION_LEVEL} from "../growth/synergyIntroductionGrant";
...
export const enhanceCard = createEnhanceCard(false);
export const enhanceSynergyIntroduction = createEnhanceCard(true);
 ...
 */
function createEnhanceCard(synergyIntroduction: boolean) {
  const command = synergyIntroduction ? "enhanceSynergyIntroduction" : "enhanceCard";
  return onCall(measuredCallable(command, async (request) => {
    ...
    if (synergyIntroduction && rule.maxLevel < SYNERGY_INTRODUCTION_LEVEL) {
    ...
    const result = await mutateSave(env, uid, command, {kind: "client", txId, ...onboardingReceipt(request.data, command)},
      ...
      // 미션 **쓰기**는 그 grants 읽기보다 뒤여야 해서 콜백 맨 끝으로 갈라 두었다.
        ...
        if (grantsReference !== null) {
          ...
          const grants = readGrants(snapshot);
          if (synergyIntroduction) {
            if (!canEnhanceSynergyIntroduction(missions.state, catalog, readOwnedIds(current.ownership),
              entries, cardId, snapshot.data()?.synergyIntroduction)) {
        ...
        let fed = feedShard(entries, cardId, step.cost, freeShot !== null, availableAmount);
        if (synergyIntroduction) {
          while (fed.level < SYNERGY_INTRODUCTION_LEVEL) {
            const remainingStep = cardEnhanceStep(rule, overrides, fed.level + 1);
            if (remainingStep === null) {
              reject("RuleUnavailable", "Card enhance rule cannot reach the synergy introduction goal.", {uid, env});
            }
            const next = feedShard(fed.entries, cardId, remainingStep.cost, true);
        ...
        if (grantsReference !== null && freeShot !== null) {
          if (synergyIntroduction) {
            transaction.set(grantsReference, {
              synergyIntroduction: {cardId, level: fed.level}, updatedAt: FieldValue.serverTimestamp(),
            }, {merge: true});
          } else {
        ...
        return {
          slots,
          wallet: nextWallet(wallet, spend(balances, step.currency, charged), command),
        };
      },
      (adopted) => {
        replayed = false;
