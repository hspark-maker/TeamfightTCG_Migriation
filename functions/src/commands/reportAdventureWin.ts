...
export const reportAdventureWin = onCall(async (request) => {
  ...
  if (chapterRows.length === 0) {
    ...
    logger.error("AdventureChapter spec is empty or unreadable",
      {...context, specRowCount: specRows.length});
    reject("SpecUnreadable", "Adventure chapter spec is unreadable.",
      {...context, specRowCount: specRows.length});
  }
  ...
  const result = await mutateSave(env, uid, "reportAdventureWin", {kind: "client", txId},
    (current): SaveMutation => {
      ...
      const pending = typeof adventure?.pendingRewardNodeId === "string" ?
        adventure.pendingRewardNodeId : "";

      if (cleared.includes(nodeId)) {
        reject("AlreadyCleared", `Adventure node '${nodeId}' is already cleared.`, {...context, pending});
      }
      // 같은 정점을 다시 신고한 것은 재시도다. 여기서 슬롯을 되쓰면 재시도마다 revision 이 올라
      // 클라가 달라진 것 없는 상태를 반복 채택한다.
      if (pending === nodeId) {
        reject("AlreadyPending", `Adventure node '${nodeId}' is already pending.`, {...context});
      }
      ...
      if (pending.length > 0) {
        ...
        reject("RewardPending", `Adventure node '${pending}' still has an unclaimed reward.`,
          {...context, pending});
      }
      ...
      if (!verdict.ok) {
        reject(verdict.reason, `Adventure node '${nodeId}' is not unlocked (${verdict.reason}).`,
      ...
      return {
        slots: {
          adventure: {
            ...
            claimedChapterIds: readNodeIdList(adventure?.claimedChapterIds),
