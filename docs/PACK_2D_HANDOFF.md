...
## ⛔ 계층·연출 부분 폐기 (2026-07-29)

**아래 §"지금 구성"의 씬 계층과 개봉 연출은 더 이상 현행이 아니다.** `PackRoot > PackVisual > BodyClip/SealClip`
구성은 봉인을 직사각형 마스크로 잘라 찢김선이 직선일 수밖에 없었고, 카드를 별도 패널 페이드로 띄워
팩과 카드가 분리돼 보였다. 2026-07-29에 **앞뒤 껍데기 사이에 카드를 끼우는 구조 + 셰이더 찢김선**으로 전면 재개발했다.

현행 진실원은 [`PACK_TEAR_GUIDE.md`](./PACK_TEAR_GUIDE.md)와 **코드**다.

**여전히 유효한 부분**: "왜 3D를 걷어냈나"(아트에 명암·크림프가 구워져 있어 2D가 더 낫다)와
아이들 플로팅·그림자 연동·스펙큘러 스윕의 근거. 이 셋은 `PackShellRig`/`PackSpecularSweep`로 이어졌다.
