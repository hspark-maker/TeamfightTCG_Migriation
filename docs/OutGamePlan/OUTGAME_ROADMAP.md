...
> ⚠️ 오른쪽 화살표(**도감 상시 생산**)는 **2026-08-14 폐기**됐다 — 도메인 C가 코드·에셋째 삭제되고 도감은 방치 생산이 없는 **카드 앨범**으로 바뀌었다. 시간 비례 공급원은 **미정**이며 후속 설계 대상이다(`OUTGAME_BOARD.md`의 `PKG-GROWTH-DIA-SUPPLY`).
...
| ~~**C**~~ | ~~도감 생산~~ ⛔ **폐기(2026-08-14)** | 행 파생 모델 · 오프라인 생산 · 완성 보상 → **카드 앨범**(`OutGame/Album/`)이 대체 | outgame-engineer |
...
### C. 도감 생산 — ⛔ **폐기(2026-08-14)**

> C-6~C-10은 구현됐다가 **코드·에셋째 삭제**됐다. 도감은 방치 생산 축을 버리고 **카드 앨범**(테마 → 페이지 → 칸, 보상 3단, `OutGame/Album/` · `UI/Album/`)으로 재구현돼 있다 — 설계·구조는 [`STRUCTURE.md`의 "신규 도감 = 카드 앨범" 절](./STRUCTURE.md). 아래 목록은 당시 계획 기록이므로 **다시 착수하지 말 것**.
...
- **F-18 도감 갤러리**: ~~세로 스크롤 3열(행 자동 확장), 행별 생산·수확~~ → **앨범으로 재정의(2026-08-14)**: 테마 갤러리(3×3) → 페이지 오버레이(3×3 칸) → 카드 상세, 보상 3단 수령 버튼.
...
공유 계약 = 여러 곳이 참조하는 표면: `CurrencyManager`·`OwnershipManager`·`CardCatalog` API, `UserSaveData` 스키마, 통합 부트 순서(`MainMenuInitializer`). 이걸 건드리면 병렬 작업들이 동시에 흔들리므로 **먼저 단독으로 얼린다.**
...
- ~~**Phase 2**: 행 완성 → 생산 시작, 디버그 시각 점프로 오프라인 누적·상한·수확 검증.~~ ⛔ **폐기(2026-08-14, 도메인 C 삭제)** — 도감 검증은 앨범 기준(페이지/테마/전체 완성 → 보상 3단 수령, 수령 낙인 재수령 불가)으로 대체된다.
