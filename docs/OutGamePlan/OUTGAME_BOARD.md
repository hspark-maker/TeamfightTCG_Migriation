...
| **PKG-TUT-GATE-SKIN** | 튜토리얼 안내 UI 프리팹화 + 사각 구멍 → **타깃 승격 + 포인터** 전환 (2026-07-31) | `OutgameTutorialBridge` 공개 API(`ShowGate`/`ShowBanner`/`HideGate`/`Clear`) **불변** · `TutorialAnchorRegistry`(무접촉) · `Runner`/`Progress`/스텝 SO **무수정** | `UI/Tutorial/OutgameTutorialGateUI.cs`(재작성) · `UI/Tutorial/OutgameTutorialBridge.cs`(필드 1 + `Ensure` 2곳) · 신규 `Assets/Assets/Prefabs/UI/Tutorial/OutgameTutorialGate.prefab` + `LobbyScene`·`CardPack` **`gatePrefab` 배선(사용자)** | PKG-OUTGAME-TUT-WIRE | 메인(코드+프리팹)+사용자(씬) | ✅ 완료(코드+프리팹) — **Unity 컴파일·씬 배선·Play 검증 대기** |
...
>
> **PKG-TUT-GATE-SKIN 결과(2026-07-31)**: "리소스 교체 가능하게 + 시각 제한 대신 포인터"라는 요청이 **정렬 순서 하드락**을 하나 드러냈다 — 딤을 500대로 올리려 했으나 `UIPoolManager` 팝업이 **400**이라, 그 위로 올리면 "유효한 덱이 없습니다"·구매 실패 팝업이 딤에 묻히고 타깃은 `interactable=true` 그대로라 소프트락 탈출로도 안 먹어 **완전 잠김**이 된다 → **350/351**로 확정하고 `Gate < UIPoolManager(400)`를 불변식으로 명문화(프리팹 값은 런타임 상수로 덮어씀). 두 번째 함정은 `ShowBanner`의 딤 — 구 코드는 "4패널 0사이즈"라는 **우연한 구현**으로 딤이 없었는데, 단일 딤으로 바꾸며 순진하게 켰다면 `PackTearHandle`의 `!IsPointerOverGameObject()` 판정이 화면 전체에서 막혀 개봉 스와이프가 영영 시작되지 않았을 것이다 → `SetDim(bool)` 명시 API로 계약화. 승격 기법 자체는 앵커 4종 조상 체인에 `Mask`/`RectMask2D`/`ScrollRect`/중첩 `Canvas`가 **하나도 없음**을 실측해 성립을 확인했고, 해제 누락(버튼이 모든 UI 위 영구 잔존)은 `Release()` 단일 창구로 봉쇄. **사용자 인계**: 두 씬 브리지의 `Gate Prefab` 필드 배선 + 인스펙터에서 딤 알파·`ringPadding`·`handOffset`·펄스 미세조정.
>
...
| **PKG-GROWTH-DIA-SUPPLY** | 🟢 | 다이아 공급 배선 | 현재 공급은 **디버그 치트뿐**(`OutgameDebugActions.GrantDiamond`). ~~`ENHANCE_ECONOMY.md`가 주 공급원으로 설계한 도감 생산 → 다이아~~ **전제 소멸(2026-08-14, 구 도감 삭제)** — `CollectionRowDef.rewardType`·`CollectionLayoutConfig`가 사라져 "저작만 하면 되는 문제"가 아니게 됐다. **공급원 자체를 새로 정해야 한다**(앨범 보상은 이미 다이아를 지급하지만 1회성이라 순환이 아니다) | WIRE | outgame-engineer + 사용자(에셋) | ⬜ 후속 — **설계 재검토 필요** |
...
| **PKG-ALBUM-CLEANUP** | 🟢 | 구 도감 삭제 | 세이브 스키마 `UserSaveData` **VERSION 1 → 2**(`collection` 슬롯 제거) | 삭제 16파일(구 `OutGame/Collection/` 테마·행·생산 축 + `UI/Collection/` 뷰 + `CollectionSaveData`) + `Tab_Collection.prefab`·`CollectionUI/` 3프리팹·`Prefabs/UI/Collection/`·`SO/CollectionLayout/`·`CollectionTest.unity` / 이동 6파일(`OutGame/Card/`·`UI/CardDetail/`·`UI/Debug/`) / 수정 6파일(`BootInstaller`·`GameManager`·`ContentProfileValidator`·`UserSaveData`·`OutgameTutorialRewind`·`DeckEditCollectionGrid`) | UI 완료 | — | ✅ **완료(2026-08-14)** |
...
>
> ~~**병존 부채(승인 시 감수하는 것)**: 카드 배치가 **두 SO에 저작된다** — 신규 `CardAlbumConfig`(앨범)와 기존 `CollectionLayoutConfig`(생산 행).~~ → **2026-08-14 해소.** `CollectionLayoutConfig`가 코드·에셋째 삭제돼 카드 배치의 저작 창구는 `CardAlbumConfig` 하나다.
>
> ~~**이중 진실원 방어 1건은 `PKG-ALBUM-DATA`에 포함한다**: 구 `CollectionThemeConfig`의 `[CreateAssetMenu]` **봉인**.~~ → **2026-08-14 해소.** `CollectionThemeConfig` 자체가 삭제돼 "카드 → 테마" 매핑이 2벌이 될 경로가 없어졌다(봉인이라는 방어선도 함께 사라졌다).
...
| 날짜 | 변경 | 영향 패키지 | 재작업? |
|---|---|---|---|
| 2026-08-14 | **구 도감(방치 생산 컬렉션) 제거 — 계약 변경 2건 + 계약 폐기 1건.** ① **세이브 스키마 `VERSION` 1 → 2**: `UserSaveData.collection` 슬롯 삭제. 지금까지의 슬롯 추가는 전부 VERSION을 유지했지만 **삭제는 버전을 올린다**(구 세이브의 `collection` 노드는 무시되고 나머지 슬롯은 그대로 읽혀 진행도 손실 없음). ② **생산 API `CollectionProductionManager` 폐기** — 계약 표에서 제거. 소비처였던 `GameManager.Flush()`의 `Save()` 호출과 `BootInstaller`의 `Init()` 호출이 함께 걷혔다. ③ **소유 API 시그니처 불변**, 다만 전체지급 창구 `OwnershipDebugTool`이 삭제되고 `OwnershipManager.GrantEntireCatalog`를 `OutgameDebugActions.UnlockAllCards`·`UnlockAllCardsButton`이 직접 부른다. ④ **부트 순서 재동결**: `BootInstaller`에서 `CatalogRows.SetLayout`/`CollectionThemes.SetSource`/`CollectionProductionManager.Init` 3줄 제거(나머지 호출 순서 무변경). ⑤ 살아남은 코드는 폴더 이동으로 이름을 정리했다 — `CardCatalog`·`OwnershipManager` → `OutGame/Card/`, `CardDetailOverlayView`·`SectionUnlockFx` → `UI/CardDetail/`, `DebugCurrencyButton`·`UnlockAllCardsButton` → `UI/Debug/`(guid 유지, 내용 무수정). `OutGame/Collection/`·`UI/Collection/` 폴더는 존재하지 않는다 | `PKG-ALBUM-CLEANUP` **완료**. `PKG-GROWTH-DIA-SUPPLY`는 **전제 소멸로 설계 재검토 필요**(도감 생산 → 다이아 경로가 사라짐) | 코드+에셋+문서 |
