...
| **PKG-ALBUM-UI** | 🟠 | 테마 목록 화면 + 페이지 오버레이 | 없음(순수 소비) | 신규 `UI/Album/`(8) + `Tab_Collection.prefab` 재구성(구 `Panel_Grid`·`Panel_ThemeBar` **비활성화**) + 테마 버튼·칸 프리팹 저작 | DATA 인터페이스 | outgame-engineer + UI | 🟩 준비(DATA 동결됨) |
...
| *(후속)* **PKG-ALBUM-CLEANUP** | 🟢 | 구 도감 삭제 | 없음 | 구 `OutGame/Collection/` 테마·행 축 + `UI/Collection/` 6파일 + `CollectionScreen.prefab` + `CollectionTest.unity` | UI 완료 | — | ⬜ **이번 스코프 밖** |

**신규 파일 14 · 수정 4파일 5줄.** 기존 도감 코드는 **한 줄도 수정하지 않는다**(프리팹 노드 비활성화만).
