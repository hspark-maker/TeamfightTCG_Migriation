...
> ⚠️ **재화 체계의 진실원은 [`CURRENCY_CYCLE.md`](./CURRENCY_CYCLE.md)로 이관됐다(2026-08-18).** 이 문서는 "도감 방치 생산 → 다이아"를 전제로 쓰였는데 그 전제가 폐기됐고, 재화도 2종에서 4종(골드·다이아·에너지·카드 조각)으로 재편됐고, 카드 강화 비용은 골드에서 카드 조각으로 옮겨갔다. 아래 사이클 전체도·건전성 표는 **옛 전제 기준**이므로 수치·경로는 새 문서를 따른다.
> 다만 *두 재화의 성격이 왜 달라야 하는가*, *강화 실패가 왜 골드 싱크의 엔진인가*, *병목이 왜 왕복해야 하는가* 는 여전히 유효한 논거라 보존한다.
