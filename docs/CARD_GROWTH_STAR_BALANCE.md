...
확정 값의 진실원은 `Assets/SO/CardGrowth/CardGrowthConfig.asset`과 카드 스펙시트다.
**§①은 에셋에 적용 완료**이고, §②·④는 아직 미적용 제안이다.
...
**적용 완료**: 마지막 단계 재화를 다이아 50 → 조각 150으로 바꿨다. 3회차만 유료 축(Diamond)이라 성격이 튀었고,
1성 이후 진화를 강화와 같은 재화 축으로 두기로 하면서 전 구간이 조각이 됐다. 성장 경로의 다이아 소모처는 이제 없다.

남은 제안:

- 강화가 3회뿐이라 실패는 두지 않는다 — 성공률 100% 유지(적용됨).
- `levelSteps.hpGain`은 `4/4/4`로 채운다(**미적용** — 에셋은 여전히 `0/0/0`). 신규 카드가 개별 커브 없이 들어왔을 때의 안전값이며, 기존 30장은 카드별 `hp2`~`hp4`가 우선하므로 수치가 바뀌지 않는다.
...
4. **조각 공급이 수요를 못 받친다.** 마지막 칸을 조각으로 바꾸면서 3성 병목이 전부 조각 한 곳으로 몰렸는데, 실공급은 도감 보상 2건이 전부다.
   - 수요: 0→3성 1장에 `25 + 75 + 150 = 250`.
   - 공급: `Assets/SO/Album/CardAlbumConfig.asset` 의 테마 보상 150 + 완주 보상 300 = **450**(디버그 지급 제외). 전 게임 조각으로 3성 1.8장.
   - 문서가 원래 전제했던 "카드팩 중복 환급"은 **작동하지 않는다** — `CardPackOpener`가 환급 재화를 지급하지 않고, 중복은 카드별 간식(`AddSnack`, 한계돌파 전용)으로만 간다. 팩 SO 7종의 `refundType: 3`(Shard)과 팩 시트의 `refundType`/`refundAmount` 열은 코드가 무시하는 죽은 저작값이다.
   - 공급원을 어디에 둘지(팩 환급 부활 / 랭크·토너먼트 보상에 조각 추가 / 도감 보상 상향)는 미결.
5. **성장 경로에서 다이아 싱크가 사라졌다.** 다이아 소모처가 성장 밖에만 남았으므로 남은 소비처 점검이 필요하다.
