...
## Android 0.0.1 리소스·APK 빌드 완료 (2026-09-16)

- 앱 버전 `0.0.1`, Android 빌드 번호 `2`, ARM64 릴리스 APK를 생성했다. 사운드 전환에 맞춰 새 버전 경로로 전체 Addressables 빌드를 수행했다.
- 카탈로그: `0.0.1/Android/catalog_2026.09.16.10.04.18.json`. 출시 상태 파일: `Build/AddressablesState/0.0.1/Android/20260916-100427-965/addressables_content_state.bin`.
- Hosting `bm-cardbattle-assets` 배포 완료(버전 `49d6054d771e94f7`). 기존 `0.0.0` 파일을 보존했다. 전체 534개 리소스의 HTTP·CORS·캐시 헤더, 신규 283개 파일의 다운로드 SHA-256과 공개 manifest 일치를 검증했다.
- 원격 번들 281개·로컬 번들 1개의 크기·CRC 검사, 자산 288개의 Provider 첫 의존 번들 내 존재 확인, `RuntimeContentCatalog` 타입 지정 로드·`Validate()` 통과. 카탈로그를 재직렬화하지 않았다.
- APK: `Build/Release-0.0.1-20260916/TeamfightTCG-0.0.1.apk`, 57,123,217 bytes. 빌드 결과 Succeeded, 오류 0개·경고 19개. SHA-256: `f6caa89d116872e4b2232b06b28805e8386227b5911e147815bfc86263f23549`.
- APK 내부 카탈로그와 배포 원본의 바이트 일치, 원격 카탈로그 URL, 로컬 raytracing 번들의 APK 포함·바이트 일치·Android 경로 모의 해석을 확인했다. 앱 빌드 중 Addressables 재생성 없음. `SpecData.bytes` SHA-256 변경 없음.
- 검증 기록: `Build/Release-0.0.1-20260916/`. 기기 설치·실행, 실제 사운드·캐시 재실행 검증은 이번 빌드 작업에서 수행하지 않았다.

## 사운드 원격 전환 구현 (2026-09-16, 위 0.0.1에 배포)

- `RemoteAudio` 그룹·라벨에 BGM 2개·효과음 28개와 설정 SO 2개를 개별 번들로 등록했다. 기존 공용 리소스 다운로드에 포함하고, 다운로드·설정 적재 후 SoundManager에 주입한다.
- 초기화 프리팹과 씬의 사운드 직접 참조를 제거했다. 신규 빌드 검사에서 원격 등록 누락과 앱 내장 참조 재발을 차단한다.
- UI 아틀라스에는 해금 안내 5개·계정 경험치 1개를 추가했다. 원격 포함 누락이 아닌 패킹 누락이었다.
- **새 앱 버전의 전체 리소스 빌드·배포와 앱 빌드가 필요하다.** 기존 APK 대상 콘텐츠 업데이트로 대신하지 않는다. 구현 당시에는 미배포였으며, 이후 위 `0.0.1` 리소스·APK 빌드를 완료했다. 기기 실행 검증은 별도다.
- 구현·검증 범위: [RemoteAudio-20260916.md](RemoteAudio-20260916.md), [UIAtlasAudit-20260916.md](UIAtlasAudit-20260916.md).
