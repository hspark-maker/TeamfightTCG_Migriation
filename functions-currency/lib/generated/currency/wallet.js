...
function normalize(balances) {
    ...
    for (const key of currencyKeys_1.CURRENCY_KEYS) {
        const value = (0, saveValues_1.intOf)(balances[key]);
        next[key] = value < 0 ? 0 : Math.min(value, currencyKeys_1.CURRENCY_MAX);
    }
    return next;
}
/**
...
function changeBalances(balances, changes) {
    ...
    for (const change of changes) {
        const moved = next[change.currency] + (0, saveValues_1.intOf)(change.amount);
        next[change.currency] = moved < 0 ? 0 : Math.min(moved, currencyKeys_1.CURRENCY_MAX);
    }
    return next;
}
/**
...
function readBalances(currency) {
    var _a;
    const source = (_a = currency === null || currency === void 0 ? void 0 : currency.balances) !== null && _a !== void 0 ? _a : {};
 ...
 */
function canAfford(balances, currency, amount) {
 ...
 */
function spend(balances, currency, amount) {
