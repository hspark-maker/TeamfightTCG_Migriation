...
namespace HeroSiege.Debugging.CheatPanel
{
    ...
    public sealed class CheatOptionEntry
    {
    ...
    public sealed class CheatOptionGroup
    {
        public string Title;
        public int Order;
        public List<CheatOptionEntry> PropertyList = new();

        // PropertyList와 같은 순서. 메서드 이름과 겹치는 토큰을 뺀 짧은 라벨(「장비 ID」→「ID」)
        public List<string> ParamLabelList = new();
        public List<CheatOptionEntry> MethodList = new();
    }

    // 서브카테고리 한 덩어리. Name이 비어 있으면 카테고리 직속이다
    public sealed class CheatOptionSubCategory
    {
        public string Name;
        public string Category;
        public List<CheatOptionGroup> GroupList = new();
    }

    public sealed class CheatOptionCategory
    {
        public string Name;
        public int OptionCount;
        public List<CheatOptionSubCategory> SubCategoryList = new();
    }

    // SROptions를 스캔해 카테고리 → 서브카테고리 → 액션 그룹 트리로 만든다.
    // UI 의존이 없어 화면 없이도 동작을 확인할 수 있다. 에디터 창의 SROptionsTreeModel과 규칙이 같지만
    // 런타임 어셈블리에 있어야 빌드에서도 쓸 수 있어 따로 둔다
    public sealed class CheatOptionTreeModel
    {
        #region Static
        ...
        // 표시명을 파라미터 라벨 계산용 토큰으로 쪼갠다. 라벨에 그대로 쓰이므로 원문 대소문자를 유지한다
        private static string[] Tokenize(string name)
        {
            return name.Split(TokenSeparatorList, StringSplitOptions.RemoveEmptyEntries);
        }
        ...
        private static bool ContainsToken(string[] tokenList, string token)
        {
            ...
            {
                if (string.Equals(other, token, StringComparison.OrdinalIgnoreCase)) return true;
            }
        ...
        private static string MakeParamLabel(CheatOptionEntry property, CheatOptionEntry method)
        {
            using (ListPool<string>.Get(out var restList))
            {
        ...
        private static int FirstOrder(CheatOptionCategory category)
        {
        ...
        private static int FirstOrder(CheatOptionSubCategory subCategory)
        {
            int order = int.MaxValue;
        ...
        private readonly List<CheatOptionCategory> categoryList = new();
        private readonly Dictionary<string, CheatOptionEntry> entryLookup = new();

        // 항목 하나가 어느 액션 카드에 속하는지. 즐겨찾기·최근에서 카드를 통째로 되찾을 때 쓴다
        private readonly Dictionary<string, CheatOptionGroup> entryGroupLookup = new();

        // MakeSubCategoryId로 만든 키 → 서브카테고리. 서브카테고리째 즐겨찾기할 때 쓴다
        private readonly Dictionary<string, CheatOptionSubCategory> subCategoryLookup = new();
        ...
        public IReadOnlyList<CheatOptionCategory> CategoryList => categoryList;
        ...
        public void Build(object optionContainer)
        {
            ...
            using (DictionaryPool<string, List<CheatOptionEntry>>.Get(out var categoryLookup))
            using (ListPool<List<CheatOptionEntry>>.Get(out var categoryEntryList))
            {
                int order = 0;
                ...
                foreach (var definition in SRDebuggerUtil.ScanForOptions(optionContainer))
                {
                    ...
                    {
                        list = ListPool<CheatOptionEntry>.Get();
                        categoryEntryList.Add(list);
                ...
                foreach (var entryList in categoryEntryList)
                    ListPool<CheatOptionEntry>.Release(entryList);
            }
        ...
        // OptionDefinition으로부터 화면 표시·검색·매칭에 쓰는 항목을 만든다
        private CheatOptionEntry CreateEntry(OptionDefinition definition, int order)
        {
        ...
        private CheatOptionCategory BuildCategory(string name, List<CheatOptionEntry> entryList)
        {
            var category = new CheatOptionCategory { Name = name, OptionCount = entryList.Count };

            using (DictionaryPool<string, List<CheatOptionEntry>>.Get(out var subLookup))
            using (ListPool<List<CheatOptionEntry>>.Get(out var subEntryList))
            {
                ...
                {
                    var subCategory = new CheatOptionSubCategory
                    {
                        Name = pair.Key,
                        Category = name,
                ...
                foreach (var subList in subEntryList)
                    ListPool<CheatOptionEntry>.Release(subList);
            }
        ...
        private List<CheatOptionGroup> BuildGroups(List<CheatOptionEntry> entryList)
        {
            var groupList = new List<CheatOptionGroup>();
            using (DictionaryPool<string, CheatOptionGroup>.Get(out var groupLookup))
            {
                foreach (var entry in entryList)
                {
                    if (entry.IsMethod == false) continue;

                    var group = new CheatOptionGroup { Title = entry.Name, Order = entry.Order };
                ...
                foreach (var entry in entryList)
                {
                    ...
                    var single = new CheatOptionGroup { Title = string.Empty, Order = entry.Order };
        ...
        private IEnumerable<CheatOptionGroup> FindOwnerGroups(CheatOptionEntry property, Dictionary<string, CheatOptionGroup> groupLookup)
        {
