...
{
    /// <param name="_turn">현재 턴 수. 코어는 턴 루프를 소유하지 않으므로 호출부(Unity의 TurnRunner,
    /// 서버 재생기의 재생 커서)가 넘긴다 — 여기서 전역을 읽으면 코어가 다시 Unity에 묶인다.</param>
    public static BattleState Capture(BattleFieldState _first, BattleFieldState _second,
        int _firstOwner, int _turn)
    {
    ...
    static void CaptureField(BattleFieldState _field, BattleState _state)
    {
        if (_field == null) return;
        if (_field.OwnerIndex < 0 || _field.OwnerIndex >= BattleState.SideCount) return;
        int t_owner = _field.OwnerIndex;
        for (int t_slot = 0; t_slot < BattleState.SlotCount; t_slot++)
        {
            CardInstance t_card = _field.GetSlot(t_slot);
        ...
        var t_fallen = new int[_field.FallenCards.Count];
        for (int t_i = 0; t_i < t_fallen.Length; t_i++)
            t_fallen[t_i] = _field.FallenCards[t_i];
        _state.FallenCardIds[t_owner] = t_fallen;
        _state.FlowStacks[t_owner] = _field.FlowStack;
    }
