...
public static class AttackProcessor
{
        ...
        bool _derivedCommand = false)
    {
        ...
        SynergyRuleTriggers.Attacked(new AttackedCtx(
            _defender, _attacker, _defenderField, _attackerField));
        // 광역 피격자도 같은 직격이므로 시너지 [Attacked]를 발화한다(패시브는 제외 — 반격은 주 대상 1장 규칙).
        if (t_splashHit)
            SynergyRuleTriggers.Attacked(new AttackedCtx(
                t_splash, _attacker, _defenderField, _attackerField));
        ...
        if (t_takesCounter)
        {
            ...
            SynergyRuleTriggers.DamageDealt(t_ctrCtx);
        }
        var t_atkCtx = new DamageDealtCtx(_attacker, _attackerField, t_actualAtkDmg, false);
        SynergyRuleTriggers.DamageDealt(t_atkCtx);
        ...
        BattleRuleBridge.ArmFinisher?.Invoke(_attackerField, _defenderField);
        ...
        BattleRuleBridge.RecordAttack?.Invoke(_attackerField.OwnerIndex, t_commandAttackerSlot,
    ...
    public static CardInstance PreSelectSplash(int _targetSlot, BattleFieldState _field)
        => PickSplash(_targetSlot, _field);

    static CardInstance PickSplash(int _targetSlot, BattleFieldState _field)
    {
        var t_adj = new List<int>();
        if (_targetSlot > 0 && _field.GetSlot(_targetSlot - 1) != null)
            t_adj.Add(_targetSlot - 1);
        if (_targetSlot < BattleFieldState.SlotCount - 1 && _field.GetSlot(_targetSlot + 1) != null)
            t_adj.Add(_targetSlot + 1);
    ...
    public static void RemoveDead(BattleFieldState _field)
    {
        for (int i = 0; i < BattleFieldState.SlotCount; i++)
        {
            ...
            if (t_c != null && !t_c.IsAlive)
            {
                ...
                SynergyRuleTriggers.Lethal(t_deathCtx);
                ...
                SynergyRuleTriggers.Removed(t_deathCtx);
                _field.RemoveCard(i);
            }
        }
    }
