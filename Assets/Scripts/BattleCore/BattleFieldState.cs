...
/// <summary>
/// 전투 필드의 가변 데이터와 그 데이터 변화에 따른 규칙 훅을 소유한다.
/// Unity 수명과 씬 배선은 <see cref="BattleField"/>가 담당한다.
...
public sealed class BattleFieldState
{
    ...
    public CardInstance SwapWithWaiting(CardInstance _outgoing)
    {
        if (!TryBeginSwapWithWaiting(_outgoing, out CardInstance t_incoming,
                out bool t_cunningReturn))
            return null;

        NotifyEntered(t_incoming);
        t_incoming.justSpawned = t_incoming.HasKeyword(CardKeyword.Invincible) || t_cunningReturn;
        CompleteSwapWithWaiting(_outgoing);
        return t_incoming;
    }
    ...
    public void SetSynergy(SynergyState _synergy) => Synergy = _synergy;

    public void NotifyPlaced(CardInstance _card)
        => SynergyRuleTriggers.Placed(new SpawnCtx(_card, this));

    public void NotifyEntered(CardInstance _card)
    {
        SynergyRuleTriggers.Entered(new SpawnCtx(_card, this));
        NotifyBoardChanged();
    }

    public void NotifyBoardChanged()
        => SynergyRuleTriggers.BoardChanged(new BoardCtx(this));
