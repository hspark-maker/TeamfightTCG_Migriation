...
public static class BattleOverForecast
{
    ...
    /// (<see cref="BattleFieldState.IsEmpty"/>가 대기열까지 보므로, 전투 종료 기준과 같은 판단이다).
    /// 슬롯 수는 한 번에 죽을 수 있는 최대치로 자른다 — 방어 측은 무쌍 광역까지 2장,
    /// 공격 측은 반격으로 공격자 1장뿐이다.</para></summary>
    public static bool CouldEnd(BattleFieldState _attackerField, BattleFieldState _defenderField, bool _peerless)
    {
    ...
    public static bool WillEnd(CardInstance _attacker, CardInstance _defender,
                               ...
                               CardInstance _preSelectedSplash, int _localOwnerIndex, out int _loserOwner)
    {
        _loserOwner = -1;
        ...
        {
            _loserOwner = _attackerField.OwnerIndex == _localOwnerIndex
    ...
    static bool WipesField(BattleFieldState _field, CardInstance _primary, int _primaryDmg,
    ...
    static bool HasLethalAlteringEffect(BattleFieldState _field, List<CardInstance> _active)
    {
