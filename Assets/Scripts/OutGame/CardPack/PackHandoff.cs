// 구매한 쪽(상점/부트)이 "이 팩을 열고, 획득 후 이 씬으로 가라(+튜토리얼이면 시작)"를 PackTest 씬에 넘기는 씬 캐리어.
...
public static class PackHandoff
{
    ...
    public static OpenedPack Pack { get; private set; }
    ...
    /// <summary>개봉 세션 컨텍스트를 싣는다. 구매 성공한 호출자만 호출(opened는 Success 전제).</summary>
    public static void Set(OpenedPack _opened, string _nextScene, bool _startTutorial)
    {
        Pack = _opened;
    ...
    /// <summary>넘겨진 세션이 있는지. PackTest 컨트롤러가 정상 진입 판별에 쓴다.</summary>
    public static bool HasPending => Pack != null;

    /// <summary>개봉 결과를 꺼내고 홀더를 통째로 비운다(1회 소비). NextScene/StartTutorial은 Consume 전에 읽을 것.</summary>
