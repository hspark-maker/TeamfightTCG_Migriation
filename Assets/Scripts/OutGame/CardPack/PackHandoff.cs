...
public static class PackHandoff
{
    ...
    public static bool IsReward { get; private set; }

    public static bool HasPending => Opened != null;

    // 개봉 세션 컨텍스트를 싣는다 (구매 성공한 호출자만)
    public static void Set(OpenedPack _opened, string _packId, string _nextScene, bool _startTutorial, bool _isReward = false)
    {
        Opened = _opened;
        PackId = _packId;
