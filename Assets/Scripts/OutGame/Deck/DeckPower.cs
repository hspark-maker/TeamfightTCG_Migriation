...
{
    /// <summary>상대 카드가 서 있는 레벨. 싱글은 랭크 티어가 정한 AI 레벨이고, 멀티는 바닥이다 —
    /// 스탯을 와이어로 보내지 않는 lockstep이라 상대 강화분을 추측하면 표시가 거짓말이 된다.</summary>
    public static int OpponentLevel
        => DeckConfig.IsMultiplayer ? CardGrowth.BaseLevel : RankManager.AiCardLevel;
    ...
    public static int LevelOf(CardData _card, bool _mine = true)
    {
        ...
        return _mine ? CardGrowthManager.GrowthOf(_card).Level : OpponentLevel;
    }
