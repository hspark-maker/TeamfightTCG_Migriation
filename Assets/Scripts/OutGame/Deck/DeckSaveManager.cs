...
public static class DeckSaveManager
{
    ...
    public static string GetName(int _index) => string.IsNullOrEmpty(s_names[_index]) ? $"덱 {_index + 1}" : s_names[_index];
    ...
    // 신규 덱이 쓸 슬롯(없으면 -1). IsSlotValid 기준이라 6장 미만으로 저장된 불완전 슬롯도 재사용 대상이 된다
    ...
    // 메모리 슬롯만 갱신한다(영속화는 SaveSlot/SaveAll).
    ...
    public static void Delete(int _index)
    {
        ...
        s_names[_index]     = "";
        s_imageKeys[_index] = "";   // 슬롯 재사용 시 새 덱이 새 이미지를 받게
        SaveSlot(_index, Enumerable.Empty<CardData>());   // 이 슬롯만 비운다(다른 슬롯 보존)
    }
