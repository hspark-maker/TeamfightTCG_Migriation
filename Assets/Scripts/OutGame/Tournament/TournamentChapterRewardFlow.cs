...
//
// 지급은 하지 않는다 — 팝업의 확인이 TournamentProgress.ClaimChapterRewardAsync를 부르고 그 안에서
...
public static class TournamentChapterRewardFlow
{
    ...
    public static async UniTask Open(int _chapterIndex)
    {
        ...
        if (!RewardClaimPopup.TryGet(out var t_popup))
        {
            ...
            await TournamentProgress.ClaimChapterRewardAsync(t_chapter.chapterId);
            return;
        }

        // 랭크·앨범·정점과 같은 규약 — [획득] 버튼 없이 배경을 눌러 받는다.
        t_popup.Show(TitleOf(t_chapter), t_lines,
            () => TournamentProgress.ClaimChapterRewardAsync(t_chapter.chapterId), _claimOnDim: true);
    }
