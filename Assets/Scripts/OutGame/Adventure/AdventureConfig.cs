...
[CreateAssetMenu(fileName = "AdventureConfig", menuName = "Card Battle/Adventure Config")]
public class AdventureConfig : ScriptableObject
{
    [Header("챕터 목록 (순서 = 진행 순서, 아래로 갈수록 뒤)")]
    [SerializeField] List<AdventureChapterDef> chapters = new List<AdventureChapterDef>();

    // 평탄화 캐시 — 최초 접근 시 구축, OnValidate에서 무효화
    bool m_built;
    List<AdventureChapterDef> m_chapters;
    List<AdventureNodeDef> m_nodes;
    ...
    public IReadOnlyList<AdventureNodeDef> Nodes
    {
        get
        {
            EnsureBuilt();
    ...
    public IReadOnlyList<AdventureChapterDef> Chapters
    {
        get
        {
            EnsureBuilt();
    ...
    public bool TryGetNode(int _index, out AdventureNodeDef _node)
    {
        EnsureBuilt();
    ...
    public bool TryGetChapter(int _chapterIndex, out AdventureChapterDef _chapter)
    {
        EnsureBuilt();
    ...
    public void FillRewards(string _nodeId, List<RewardLine> _sink)
    {
        ...
        if (AdventureSpec.TryGetRewards(_nodeId, out List<AlbumRewardDef> t_spec)) FillFrom(t_spec, _sink);
    }
    ...
    public void FillChapterRewards(string _chapterId, List<RewardLine> _sink)
    {
        ...
        if (AdventureSpec.TryGetRewards(_chapterId, out List<AlbumRewardDef> t_spec)) FillFrom(t_spec, _sink);
    }
    ...
    void EnsureBuilt()
    {
        ...
        if (m_chapters == null) m_chapters = new List<AdventureChapterDef>();
        if (m_nodes == null) m_nodes = new List<AdventureNodeDef>();
        ...
        {
            AdventureChapterDef t_chapter = m_chapters[t_c];
    ...
    {
        if (string.IsNullOrEmpty(_nodeId) || _enemyDeckIds == null) return false;

        for (int t_c = 0; t_c < chapters.Count; t_c++)
        {
            AdventureChapterDef t_chapter = chapters[t_c];
            if (t_chapter.nodes == null) continue;

            for (int t_n = 0; t_n < t_chapter.nodes.Count; t_n++)
            {
                AdventureNodeDef t_node = t_chapter.nodes[t_n];
                if (!string.Equals(t_node.nodeId, _nodeId, StringComparison.Ordinal)) continue;

                t_node.enemyDeckIds = new List<int>(_enemyDeckIds);
                t_node.aiCardLevel = _aiCardLevel;
                t_chapter.nodes[t_n] = t_node;
                chapters[t_c] = t_chapter;
                m_built = false;
                return true;
            }
        }
    ...
    void ValidateAdventure() => AdventureValidator.Validate(this);
#endif
}
