...
// 트리거 문은 졸업 낙인이 찍혀야 열리므로 지금은 둘이 겹치지 않는다.
// 그래도 트리거를 먼저 보는 순서는 남긴다 — 겹치는 순간이 생기면 화면을 쥔 쪽이 언제나 트리거이고
// (로비 안에서 시작해 로비 안에서 끝난다), 뒤집으면 온보딩 스텝이 트리거 스텝을 가려
// freeOfCharge·anchorCard가 조용히 무시된다.
