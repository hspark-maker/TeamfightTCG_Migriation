...
public static partial class SpecFirestoreUploader
{
    ...
    public static List<string> ListTables(out string _error)
    {
        ...
        if (!t_names.Contains("CardCraft")) t_names.Add("CardCraft");
        if (!t_names.Contains("CosmeticItem")) t_names.Add("CosmeticItem");
        if (!t_names.Contains("Title")) t_names.Add("Title");
    ...
    static bool TryBuildSnapshot(object _manager, string _table, out TableSnapshot _snapshot, out string _error)
    {
        ...
        if (_table == "Mission" || _table == "AccountLevel" || _table == "Reward" || _table == "Achievement" ||
            _table == "CardCraft" || _table == "CosmeticItem" || _table == "Title")
        ...
        out string _tablesJson, out string _error)
    {
        ...
        foreach (string t_table in SpecPayloadCodec.PublishedTableNames)
            // 서버 전용 기능 도입 전 릴리스도 되돌릴 수 있다. 새 발행 시에는 모두 필수로 검증한다.
            if (t_table != "Achievement" && t_table != "CardCraft" && t_table != "CosmeticItem" && t_table != "Title" &&
                _tablesJson.IndexOf("\"" + t_table + "\":", StringComparison.Ordinal) < 0)
            {
