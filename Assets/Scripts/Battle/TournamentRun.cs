...
/// 활성 시: 적 카드 레벨이 랭크가 아니라 정점 저작값으로 고정되고(부트 초기화(InitializationRunner)의 EnemyGrowthProvider),
/// 전투 결과가 랭크 대신 <c>TournamentResultHandoff</c>로 나간다(TurnRunner.CaptureResult).
/// 수명 종점은 TurnRunner.Cleanup — 로비에서 진입을 취소한 경로는 씬 전환이 없으므로 진입점이 직접 End한다.
/// </summary>
public static class TournamentRun
{
