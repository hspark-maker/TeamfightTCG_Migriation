...
public static class BattleGoldenRecorder
{
    ...
    /// 검증되지 않는다.</summary>
    public static void Finish(bool _localWon)
    {
        if (s_document == null) return;
    ...
    static void FinishCore(bool _localWon)
    {
        ...
        s_document.winnerOwner = _localWon ? t_localOwner : 1 - t_localOwner;
    ...
    static BattleGoldenDeck CaptureDeck(BattleField _field)
    {
        ...
        return new BattleGoldenDeck { ownerIndex = _field.OwnerIndex, cards = t_cards.ToArray() };
    }
