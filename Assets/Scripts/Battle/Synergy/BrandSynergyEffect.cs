...
[CreateAssetMenu(fileName = "BrandSynergyEffect", menuName = "Card Battle/Synergy Effect/Brand")]
public class BrandSynergyEffect : SynergyEffect
{
    ...
    public override async UniTask OnBeforeAttack(BeforeAttackCtx _ctx)
    {
        ...
        // 실제 적용량은 ClampDamage가 유일 진실원 — 비늘 감소와 체력 상한이 여기서 이미 반영된다.
        // (발당 1씩 띄우면 감소가 걸렸을 때 숫자 합이 실제 피해와 어긋난다.)
        int t_applied   = _ctx.defender.ClampDamage(t_totalDamage);
        ...
        _ctx.defender.TakeDamage(t_totalDamage);   // 합산 선피해 1회: 보호막·비늘도 정확히 1회만 판정한다.
