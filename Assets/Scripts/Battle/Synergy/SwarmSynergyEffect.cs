...
public class SwarmSynergyEffect : SynergyEffect
{
    ...
    public override UniTask OnBeforeAttack(CardInstance self, CardInstance defender, BattleField field, SynergyData synergy)
    {
        if (defender == null || !defender.IsAlive || field == null) return UniTask.CompletedTask;

        int t_count = 0;
        foreach (var t_card in field.GetActiveCards())   // 슬롯 라이브 카드(공격자 포함)
        {
            if (t_card == null || !t_card.IsAlive) continue;
            if (SynergyApplier.BelongsTo(t_card, synergy)) t_count++;
        }

        t_count = Mathf.Min(t_count, BattleField.SLOT_COUNT);   // 방어적 상한(아군 슬롯 ≤3 → 자연히 만족, 회귀 가드)
        if (t_count > 0)
        {
            defender.TakeDamage(t_count, true);   // 선피해도 공격 직격 취급: 비늘 감소 대상. 규칙 전량 TakeDamage 위임.
            SynergyTriggers.Fire(self, synergy);   // 선피해 발동 시에만 배너+배지 pop(스팸 방지)
        }
