...
public class SwarmSynergyEffect : SynergyEffect
{
    ...
    public override UniTask OnBeforeAttack(BeforeAttackCtx _ctx, SynergyData _synergy)
    {
        ...
        foreach (var t_card in _ctx.ownField.GetActiveCards())   // 슬롯 라이브 카드(공격자 포함)
        {
            ...
            if (SynergyApplier.BelongsTo(t_card, _synergy)) t_count++;
        }
        ...
        if (t_count > 0)
        {
            ...
            SynergyTriggers.Fire(_ctx.self, _synergy);   // 선피해 발동 시에만 배너+배지 pop(스팸 방지)
        }
