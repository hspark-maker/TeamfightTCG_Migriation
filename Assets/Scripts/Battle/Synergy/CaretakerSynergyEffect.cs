...
public class CaretakerSynergyEffect : SynergyEffect
{
    ...
    public override UniTask OnSpawn(CardInstance self, BattleField field, SynergyData synergy)
    {
        ...
        if (self == null || !self.IsAlive || field == null) return UniTask.CompletedTask;
        if (!SynergyApplier.BelongsTo(self, synergy)) return UniTask.CompletedTask;

        foreach (var t_card in field.GetActiveCards())   // 자신 포함 라이브 슬롯 카드
        {
            if (t_card == null || !SynergyApplier.BelongsTo(t_card, synergy)) continue;
            t_card.Heal(amount);
            t_card.GrantBonusHp(amount);
        }
        SynergyTriggers.Fire(self, synergy);   // 스폰 주체(self) 기준 1회 배너+배지 pop(동료 전원 반복 금지)
