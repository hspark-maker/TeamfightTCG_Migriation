...
/// <summary>해금 안내 한 줄. 글자는 KeywordExplainItem에 맡기고 데모 띠·시너지 배지·티어 줄을 채운다.</summary>
...
public class UnlockIntroRow : MonoBehaviour
{
    ...
    [Tooltip("데모 띠의 AspectRatioFitter(선택). 맞춤 모드는 저작이 갖고 코드는 실측 비율만 넣는다.")]
    ...
    // 배지와 데모 띠는 같은 자리를 쓰므로 둘 중 하나만 세운다(판정은 SetDemo).
    Sprite m_badgeIcon;
    ...
    /// <summary>글자를 채우고 배지 후보를 받아 둔다(가운데 자리의 주인은 SetDemo).</summary>
    public void Bind(UnlockIntro _intro)
    {
        ...
        // 데모 여부가 아직 안 정해졌다 — SetDemo를 기다린다
    ...
    /// <summary>데모 띠에 그림을 물린다. null이면 띠를 끄고 시너지 줄은 그 자리에 배지를 대신 세운다.</summary>
    ...
    // 티어 줄을 위에서부터 채우고 남는 줄은 끈다(키워드 줄은 티어가 없어 전부 꺼진다).
    ...
    // 조건이 SetSynergyBadge와 뒤집혀 있는 것이 규약이다 — 둘이 같이 뜨면 같은 아이콘이 두 장이 된다.
