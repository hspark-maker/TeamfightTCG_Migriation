...
// 잠김 판이 걷힌 뒤 그 아래 내용이 칩 줄 → 설명 순으로 들어오는 연출.
// 자리 대신 알파·배율만 건드린다 — 레이아웃 그룹이 anchoredPosition을 매 프레임 되돌린다.
public class SectionRevealFx : MonoBehaviour
{
    ...
    /// <summary>한 번 돈다. 미배선 축은 빠지고, 둘 다 없으면 null.</summary>
    public Tween Play()
    {
    ...
    void OnDisable()
    {
        // 연출 도중 꺼지면 마무리 콜백이 오지 않아 반투명하게 굳는다.
        KillRunning();
        RestoreAuthored();
    }
