...
public sealed class DeckEditData : UIData
{
    ...
    /// <summary>하단 덱 선택 바. 로비 덱 탭은 목록 뎁스가 없어 여기서 덱을 갈아탄다 —
...
public class DeckEditController : PooledUIBase, IPointerClickHandler
{
    ...
    // 슬롯 선택 모드에서 교체될 차례를 기다리는 카드. 0이면 평상시다.
    // 편성 진실원(m_working)과 달리 이건 화면 조작 상태라 저장·dirty와 무관하다.
    int m_pendingSwapCard;

    // 직전 ApplySlotPickVisual이 칠한 논리 상태. 되칠할 때 트윈을 다시 걸지 말지를 여기서 가른다.
    bool m_swapVisualOn;

    // 드래그가 켜 둔 칸 강조. 탭 교체(m_pendingSwapCard)와 배타다 — 드래그 개시가 탭 모드를 내린다.
    ...
    // 만석 삭제 모드에서는 편집 중인 덱까지 지울 수 있고, 그 앞 덱을 지우면 압축 당김으로 좌표가 밀린다 —
    // 편집 중인 덱을 참조로 붙들었다가 삭제 뒤 좌표를 되찾고, 그 덱 자체가 지워졌으면 이웃 덱으로 갈아탄다
    // (OnDeckStripSlotClicked와 같은 참조 판정 · RelocateEditingSlot).
    ...
    void OnDeckStripSlotDelete(int _slotIndex)
    {
        ...
        UIPoolManager.Instance.AddOrUpdateUI<SimpleYNPopup>(new SimpleYNPopupData
        {
            ...
            yesAction = () =>
            {
                ...
                // 갈아탔으면 Open 이 바를 이미 다시 세웠고, 닫혔으면 세울 화면이 없다.
                if (t_editing != null && RelocateEditingSlot(t_editing)) return;
                if (!IsOpen) return;
    ...
    // 편집 중인 덱 자체가 지워졌으면 같은 자리로 당겨 온 덱(없으면 마지막 덱)으로 갈아탄다 —
    // 이 화면은 돌아갈 목록이 없어 편집기를 닫으면 덱 탭을 통째로 떠나게 된다.
    // 갈아탔으면 true 를 돌려준다(Open 이 바를 다시 세웠다는 뜻).
    bool RelocateEditingSlot(List<int> _editing)
    {
        ...
        int t_next = -1;
        for (int t_i = Mathf.Min(this.m_slotIndex, DeckSaveManager.SLOT_COUNT - 1); t_i >= 0; t_i--)
        {
        ...
        if (t_next >= 0)
        {
            // Open 은 m_dirty 를 내리고 바를 다시 세운다 — 지워진 덱의 미저장 편집분은 함께 버려진다.
            ConsumeHoldout();   // 덱을 갈아타는 경로는 전부 이 요청을 소비한다(OnDeckStripSlotClicked와 같다)
            Open(t_next);
            return true;
        }

        // 남은 덱이 없다(만석에서 하나만 지우므로 정상 경로에서는 오지 않는다). 빈 좌표에 저장하게 둘 수 없으니 나간다.
        // 좌표를 먼저 비운다 — 로비 임베디드 경로의 HideEditor 는 CurrentSlot 을 대표 덱 선택에 되넘기므로,
        // 낡은 좌표를 남기면 엉뚱한 덱이 대표가 된다.
        Debug.LogWarning("[DeckEditController] 갈아탈 덱이 없음 — 편집기를 닫는다.");
        this.m_slotIndex = -1;
        ExitEditor();
        return false;
    }
    ...
    {
        if (m_pendingSwapCard <= 0) return;   // 평상시 클릭마다 그리드 전체를 되칠할 이유가 없다
    ...
    // 둘레에 발광을 켠다. 슬롯 재바인딩(Bind)과 컬렉션 착용딤이 이 표시를 원상복구하므로 반드시 그 뒤에 부른다.
    void ApplySlotPickVisual(bool _forceInstant = false)
    {
        bool t_picking = m_pendingSwapCard > 0 || m_dragSwapVisual;
    ...
    void ClearSlotPickVisual()
    {
        ...
        if (collectionGrid != null) collectionGrid.SetPickedCard(0);
    }
    ...
    void OnSlotClicked(int _slotIndex)
    {
        ...
        int t_pushed = _slotIndex >= 0 && _slotIndex < m_working.Length ? m_working[_slotIndex] : 0;
    ...
    void FlyPushedCardToCollection(int _slotIndex, int _pushed)
    {
        if (_pushed <= 0 || dragController == null || collectionGrid == null) return;
    ...
    void RefreshAll()
    {
        ...
        if (m_pendingSwapCard > 0 || m_dragSwapVisual) ApplySlotPickVisual();
    }
            ...
            ? new SimpleYNPopupData
              {
                  ...
                  // MarkSaved까지 태워야 dirty가 내려간다 — 안 그러면 저장이 끝났는데도 저장 버튼이 살아 있고,
                  // 스트립이 새 이름·새 좌표를 모른 채 남아 다음 이탈에서 확인 팝업이 또 뜬다.
                  yesAction = () => { if (SaveIfComplete()) { MarkSaved(); _onGranted?.Invoke(); } },
                  noText    = "저장 안 함",
