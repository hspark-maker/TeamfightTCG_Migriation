...
{
    // 커튼은 씬 전환 직전에 동기 생성되어야 하므로 Initialize 프리팹이 직렬화한 카탈로그에서 직접 얻는다.
    ...
    /// <summary>씬을 벗어날 때 BGM이 빠지는 시간(초). 커튼이 닫히는 0.22초에 맞춘 길이다 —
    /// 소리가 커튼과 같이 빠져야 화면만 덮이고 소리는 남는 꼴이 안 된다.
    ///
    /// <para>커튼을 거치지 않고 씬을 벗어나는 경로(MatchSceneCarrier)도 같은 길이를 써야 하므로 열어 둔다 —
    /// 값이 갈리면 전환마다 소리가 다른 박자로 빠진다.</para></summary>
    public const float BgmFadeOutSeconds = 0.25f;
    ...
    {
        // 커튼으로 씬을 벗어나는 호출부는 전부 이 창구를 지나므로 BGM 퇴장을 여기서 책임진다.
        // 커튼을 안 쓰는 경로(MatchSceneCarrier)는 자기 창구에서 같은 상수로 부른다.
