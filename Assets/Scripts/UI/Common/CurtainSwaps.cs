...
// 화면 내 교체. 씬은 그대로 두고 커튼 밑에서 오버레이만 갈아치운다(매칭 화면 → 덱 화면).
public class ScreenSwap : ICurtainSwap
{
    readonly Action m_swap;

    bool m_done;

    public ScreenSwap(Action _swap)
    {
        m_swap = _swap;
    }
    ...
    public IEnumerator Commit()
    {
        ...
        yield return null;   // 새 화면이 최소 한 번 그려진 뒤에 열리도록 한 프레임 양보
    }

    // 닫히는 도중 잘리면 이전 화면이 남고 다음 화면은 서지 않는다 — 아무 화면도 없는 곳에 갇히느니
    // 하드컷으로라도 갈아치운다.
    public void Abort()
    {
        Run();
