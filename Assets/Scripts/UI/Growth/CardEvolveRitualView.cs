...
public class CardEvolveRitualView : CardGrowthRitualView
{
    ...
    [Range(0f, 1f)] [SerializeField] float overheatStart = 0.35f;               // 충전의 어디서부터 면이 덮이나. 앞쪽은 테두리 빛만 올라 '가장자리부터'가 된다
    ...
    [Header("표면을 훑는 빛")]
    [SerializeField] float gleamDelay = 0.2f;                                   // 빛이 걷히고 얼마나 뒤에 훑나. 겹치면 백열에 묻힌다
    [SerializeField] float gleamSweep = 0.45f;

    [Header("박자")]
    [SerializeField] float enterDuration  = 0.18f;
    [SerializeField] float chargeDuration = 0.80f;                              // 금빛이 차오르는 동안
    [SerializeField] float blazeDuration  = 0.35f;                              // 백열이 카드를 마저 삼키는 동안
    [SerializeField] float holdDuration   = 0.25f;                              // 완전히 덮인 채 머무는 한 박. 이 정적이 진화의 무게다(담금질은 0.05다)
    [SerializeField] float revealDuration = 0.60f;                              // 빛이 물러나며 새 모습이 드러나는 시간. 담금질의 회수(0.4)보다 길어야 '드러난다'가 된다
    ...
    [SerializeField] float liftDistance = 12f;                                  // 충전 동안 떠오르는 높이(px)
    ...
    [SerializeField] float burstSettle  = 0.55f;                                // 회수 시간. 빛이 물러나는 속도와 같아야 '빛이 카드가 되었다'로 읽힌다
    ...
    [Range(-1f, 1f)] [SerializeField] float chargeDim = 0.3f;
    ...
    // 결과 구간은 회수와 빛줄기가 다 지나갈 자리를 담아야 한다 — 짧으면 복귀가 그 위를 덮친다.
    ...
    protected override float BuildStage(Sequence _seq, EEnhanceOutcome _outcome, bool _chained)
    {
        ...
        float t_resultDur = Mathf.Max(Mathf.Max(0.05f, this.revealDuration), Mathf.Max(this.resultHold, RevealSettle));
    ...
    void BuildEnter(Sequence _seq, float _at, float _dur, bool _chained)
    {
        ...
        // 앞 결과가 남긴 표면·자세를 이 구간이 데려온다("한 번 더"로 이어온 길).
        _seq.Insert(_at, this.cardStage.DOAnchorPos(this.m_baseAnchored, _dur).SetEase(Ease.OutQuad));
        _seq.Insert(_at, this.shading.TweenHeat(0f, _dur).SetEase(Ease.OutQuad));
    ...
    {
        _seq.Insert(_at, this.cardStage.DOAnchorPosY(this.m_baseAnchored.y + this.liftDistance, _dur).SetEase(Ease.OutQuad));
        ...
        _seq.Insert(_at, this.dimTint.TweenLevel(this.chargeDim, _dur).SetEase(Ease.InQuad));
    ...
    void BuildReveal(Sequence _seq, float _at, float _hold)
    {
        ...
        // 백열이 걷히며 카드가 드러난다. 열은 잔열만 남기고 천천히 식는다.
        _seq.Insert(_at, this.shading.TweenBlind(0f, t_out).SetEase(Ease.OutQuad));
        _seq.Insert(_at, this.shading.TweenCover(0f, t_out * 0.7f).SetEase(Ease.OutQuad));
    ...
    // 표면을 한 번 훑는 빛. 갓 태어난 것을 닦아 내는 마지막 획이다.
