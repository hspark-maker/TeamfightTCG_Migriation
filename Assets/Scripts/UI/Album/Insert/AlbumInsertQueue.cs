...
{
    static readonly List<CardData> s_pending = new List<CardData>();

    public static bool HasPending => s_pending.Count > 0;

    // 세션 도달 전 연속 개봉도 남도록 누적한다.
    public static void Enqueue(IReadOnlyList<CardData> _cards)
    {
    ...
    // 1회 소비 — 꺼내면 홀더는 비워진다.
    public static bool TryConsume(out IReadOnlyList<CardData> _cards)
    {
        ...
        _cards = new List<CardData>(s_pending);
        s_pending.Clear();
        return true;
    }
