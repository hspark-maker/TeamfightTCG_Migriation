...
public static class PackPurchaseFlow
{
    ...
    public static async UniTask<OpenedPack> PurchaseAsync(string _packId, object _owner, bool _callerOwnsWaiting = false)
    {
        OpenedPack t_opened;

        s_inFlight++;
        if (!_callerOwnsWaiting) ServerWaitOverlay.Hold(_owner);
        try
        {
        ...
        {
            if (!_callerOwnsWaiting) ServerWaitOverlay.Release(_owner);
            s_inFlight--;
        }
        ...
        if (!_callerOwnsWaiting) PackPurchaseFailurePopup.Show(_packId, t_opened.Result);
        return null;
    }
