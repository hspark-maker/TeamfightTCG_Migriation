...
//
// 팝업을 세우는 일은 맵이 진다(AdventureMapOverlayView.OpenReturnReward).
public static class AdventureReturnFlow
{
    // 격파 신고를 기다리는 동안의 대기 표시 owner. static 흐름이라 타입 자체를 키로 쓴다.
    static readonly object WAIT_OWNER = typeof(AdventureReturnFlow);
    ...
    {
        if (!AdventureResultHandoff.TryConsume(out string t_nodeId, out bool t_won)) return;
    ...
    // 그럴 때는 왕복 없이 그 프레임에 연다 — 이것이 "복귀하면 곧바로"를 지키는 자리다.
    static async UniTaskVoid ClaimWhenReported(string _nodeId)
    {
        await ReportIfNotStampedAsync(_nodeId);
    ...
    // 낙인이 이미 서 있으면 왕복이 없다 — 그 갈래에서는 대기막도 걸지 않는다(한 프레임 깜빡이면 결함으로 읽힌다).
    //
    // 이 자리를 덮는 이유: 복원(ReturnRequested)이 먼저 나가 맵이 이미 떠 있고 조작까지 열려 있다.
    // 그 사이 유저가 탭을 옮기거나 다른 정점을 누르면 OpenReturnReward 가 무대를 잃어,
    // 방금 이긴 보상이 손으로 다시 찾아 눌러야 하는 자리로 떨어진다.
    static async UniTask ReportIfNotStampedAsync(string _nodeId)
    {
        if (IsRewardPending(_nodeId)) return;

        // 수령 왕복(AdventureRewardFlow)과 owner 를 가른다 — 키를 공유하면 먼저 끝난 쪽이 남은 차단막까지 걷는다.
        ServerWaitOverlay.Hold(WAIT_OWNER);
        try
        {
            await AdventureWinCommand.ReportWinAsync(_nodeId);
        }
        finally
        {
    ...
    static bool IsRewardPending(string _nodeId)
    {
        ...
        return t_index >= 0 && AdventureProgress.IsRewardPending(t_index);
    }
}
