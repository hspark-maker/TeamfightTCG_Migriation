...
//
// 지급은 하지 않는다 — 팝업의 확인이 _onConfirm을 부르고 그 안에서 AdventureProgress.ClearNodeAsync가
...
public static class AdventureRewardFlow
{
    ...
    public static bool Open(string _nodeId, Action _onClaimed, Action _onClosed = null)
    {
        ...
        if (AdventureProgress.IsCleared(_nodeId)) return false;
        ...
        AdventureProgress.FillRewards(t_index, t_lines);
    ...
    static async UniTask<RewardClaimOutcome> ClaimThenNotifyAsync(string _nodeId, Action _onClaimed)
    {
        ...
        try
        {
            t_outcome = await AdventureProgress.ClearNodeAsync(_nodeId);
        }
        finally
        {
            s_inFlight--;
