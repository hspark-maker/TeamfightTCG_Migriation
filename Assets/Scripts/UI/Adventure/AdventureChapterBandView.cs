...
//
// 정점 뷰와 같은 규약 — 인덱스만 들고 표시값은 매번 AdventureProgress에서 다시 받는다(스냅샷은 클리어 후 stale).
public class AdventureChapterBandView : MonoBehaviour
{
    ...
    public void Refresh()
    {
        ...
        if (!AdventureProgress.TryGetChapter(this.m_index, out AdventureChapterDef t_chapter)
            || !AdventureProgress.TryGetChapterProgress(this.m_index, out int t_cleared, out int t_total))
        {
        ...
        bool t_rankLocked = AdventureProgress.IsChapterRankLocked(this.m_index) && t_cleared < t_total;
        ...
        bool t_claimable = AdventureProgress.CanClaimChapterReward(this.m_index);
    ...
    void ApplyRankLock(bool _locked, int _chapterIndex)
    {
        ...
        if (!AdventureProgress.TryGetRequiredGrade(_chapterIndex, out ERankGrade t_grade)
    ...
    void ApplyIntroLocked()
    {
        ...
        if (AdventureProgress.IsChapterRankLocked(this.m_index))
        {
    ...
    async UniTaskVoid OpenClaimAsync()
    {
        ...
        await AdventureChapterRewardFlow.Open(this.m_index);
    }

    void BindRewardSlots()
    {
        AdventureProgress.FillChapterRewards(this.m_index, s_rewardBuffer);
