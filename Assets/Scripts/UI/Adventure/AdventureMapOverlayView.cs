...
public class AdventureMapOverlayView : MonoBehaviour, IPointerClickHandler
{
    ...
    [SerializeField] AdventureNodeView nodePrefab;
    ...
    [SerializeField] AdventureChapterBandView bandPrefab;
    ...
    readonly List<AdventureNodeView> m_nodes = new List<AdventureNodeView>();
    ...
    readonly List<AdventureChapterBandView> m_bands = new List<AdventureChapterBandView>();
    ...
    public void Close()
    {
        OutgameTutorialRunner.AbortGuided(EOutgameTutorialTrigger.AdventureMapFirstOpen);   // 화면 이탈 = 이번 세션 미루기
    ...
    public void OpenReturnReward(string _nodeId)
    {
        ...
        if (this.m_claimSeq != null || AdventureRewardFlow.IsClaiming) return;
        ...
        int t_index = AdventureProgress.IndexOf(_nodeId);
        if (t_index < 0 || t_index >= this.m_nodes.Count) return;
        if (!AdventureProgress.IsRewardPending(t_index)) return;
    ...
    void Build()
    {
        ...
        int t_count = AdventureProgress.NodeCount;
        var t_positions = new Vector2?[t_count];
        var t_bandBottoms = new float?[AdventureProgress.ChapterCount];
        ...
        for (int t_i = 0; t_i < t_count; t_i++)
        {
            ...
            AdventureNodeView t_node = Instantiate(this.nodePrefab, this.content);
    ...
    float BuildChapterTiles(Vector2?[] _positions, float?[] _bandBottoms)
    {
        ...
        int t_nodeStart = 0;
        int t_chapters = AdventureProgress.ChapterCount;

        for (int t_c = 0; t_c < t_chapters; t_c++)
        {
            if (!AdventureProgress.TryGetChapter(t_c, out AdventureChapterDef t_chapter)) continue;
            ...
            if (t_chapter.tilePrefab == null || !(t_chapter.tilePrefab.transform is RectTransform))
                this.ErrorChapter(t_c, "쓸 수 있는 tilePrefab이 없다 — 이 챕터의 정점과 길이 화면에 나오지 않는다. AdventureConfig에 타일을 저작할 것.");
            else
            {
    ...
    float PlaceChapterTile(int _chapterIndex, AdventureChapterDef _chapter, float _cursor, int _nodeStart, Vector2?[] _positions)
    {
    ...
    void BuildChapterBands(float?[] _bandBottoms)
    {
        ...
        for (int t_c = 0; t_c < _bandBottoms.Length; t_c++)
        {
            ...
            AdventureChapterBandView t_band = Instantiate(this.bandPrefab, this.content);
    ...
    void RefreshLinks()
    {
        for (int t_i = 0; t_i < this.m_links.Count; t_i++)
        {
            ...
            Color t_color = AdventureProgress.StateOf(t_link.Index) == EAdventureNodeState.Cleared
                ? this.clearedColor
                : this.lockedColor;
    ...
    {
        string t_line = $"[Adventure] 챕터 #{_chapterIndex}: {_message}";
    ...
    void RefreshNodes()
    {
        ...
        if (this.m_suspendRefresh && this.m_claimSeq == null && !this.IsIntroPending && !AdventureRewardFlow.IsClaiming)
    ...
    void ApplyTutorialAnchor(bool _on)
    {
        ...
        if (_on)
        {
            int t_current = AdventureProgress.CurrentNodeIndex;
            if (t_current >= 0
                && AdventureProgress.CanEnter(t_current)
                && !AdventureProgress.IsRewardPending(t_current))
                t_target = t_current;
        }
    ...
    {
        int t_index = AdventureProgress.FocusNodeIndex;
        if (t_index >= 0)
        {
            this.ScrollToNode(t_index);
            return;
    ...
    void OnNodeTapped(int _index)
    {
        ...
        if (AdventureRewardFlow.IsClaiming) return;

        // 선물은 도전이 아니라 수령이다 — 전투로 잇지 않고 보상 팝업으로 간다.
        if (AdventureProgress.IsRewardPending(_index))
        {
            this.OpenNodeReward(_index);
            return;
        }

        if (!AdventureProgress.CanEnter(_index)) return;
    ...
    {
        if (!AdventureProgress.TryGetNode(_index, out AdventureNodeDef t_node)) return;
        ...
        if (!AdventureRewardFlow.Open(t_node.nodeId,
    ...
    void PlayClaimSequence(int _index)
    {
        ...
        if (!AdventureProgress.TryGetNode(_index, out AdventureNodeDef t_node)
            || !AdventureProgress.IsCleared(t_node.nodeId))
        {
            this.AbortClaimSequence();
            return;
        }
        ...
        // 이 정점의 도장은 낙관 시점에 이미 꽂혔으므로 여기서 다시 부르지 않는다(AdventureNodeView.Refresh).
        this.RefreshNodes();
    ...
    void AbortIfIdle()
    {
        if (AdventureRewardFlow.IsClaiming || this.m_claimSeq != null || this.m_litLink < 0) return;
    ...
    bool CanCollectUnlockIntro
        => string.IsNullOrEmpty(AdventureProgress.PendingRewardNodeId)
           && !AdventureRewardFlow.IsClaiming
           && this.m_claimSeq == null;
    ...
    void CollectUnlockIntro()
    {
        ...
        AdventureProgress.BackfillSeenUnlocks();

        if (!AdventureProgress.TryTakeUnlockShowcase(out AdventureUnlockShowcase t_showcase)) return;
    ...
    void GetChapterNodeRange(int _chapterIndex, out int _start, out int _count)
    {
        ...
        {
            int t_nodes = AdventureProgress.TryGetChapter(t_c, out AdventureChapterDef t_chapter) ? t_chapter.NodeCount : 0;
    ...
    float ChapterStepsEnd(int _chapterIndex, float _at, float _step)
    {
        ...
        for (int t_i = 0; t_i < t_count; t_i++)
        {
            ...
            AdventureNodeView t_view = t_node < this.m_nodes.Count ? this.m_nodes[t_node] : null;
    ...
    void CommitUnlockSeen()
    {
        ...
        AdventureProgress.MarkUnlockSeen(new AdventureUnlockShowcase(t_chapters, t_nodes));
    }
