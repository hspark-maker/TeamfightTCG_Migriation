namespace SRDebugger.Internal
{
    ...
    public static class OptionControlFactory
    {
        ...
        {
            // 동적 드롭다운은 타입이 아니라 어트리뷰트로 고른다 — int 프로퍼티가 NumberControl 로 가기 전에 가로챈다
            if (SRDynamicDropdown.IsDynamicDropdown(from.Property))
            {
                return _dataControlPrefabs.FirstOrDefault(p => p is DynamicDropdownControl);
            }
