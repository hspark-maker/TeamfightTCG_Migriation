...
namespace SRDebugger.Internal
{
    ...
    public static class SRDebuggerUtil
    {
        ...
        public static List<OptionDefinition> ScanForOptions(object obj)
        {
            ...
            var categoryField = obj.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Static);
            
            // SR카테고리를 Open 정보 딕셔너리로 생성
            Dictionary<string, bool> categoryOpenDict = new();
            foreach (var fieldInfo in categoryField)
            {
                var categoryOpenAttribute = SRReflection.GetAttribute<SROpenAttribute>(fieldInfo);
                var fieldValue = fieldInfo.GetValue(obj) as string;
                if(string.IsNullOrEmpty(fieldValue)) continue;
                categoryOpenDict[fieldValue] = categoryOpenAttribute is { IsOpen: true };
            }
            
            // SR Sub 카테고리를 Open 정보 딕셔너리로 생성
            Dictionary<string, bool> subCategoryOpenDict = new();
            foreach (var fieldInfo in categoryField)
            {
                var categoryOpenAttribute = SRReflection.GetAttribute<SRSubOpenAttribute>(fieldInfo);
                var fieldValue = fieldInfo.GetValue(obj) as string;
                if(string.IsNullOrEmpty(fieldValue)) continue;
                subCategoryOpenDict[fieldValue] = categoryOpenAttribute is { IsOpen: true };
            }
            ...
            foreach (var memberInfo in members)
            {
                ...
                var subCategoryAttribute = SRReflection.GetAttribute<SRSubCategoryAttribute>(memberInfo);
                var subCategory = subCategoryAttribute == null ? string.Empty : subCategoryAttribute.SubCategory;
                ...
                var open = categoryOpenDict.GetValueOrDefault(category, true) && subCategoryOpenDict.GetValueOrDefault(subCategory, true);
                ...
                if (memberInfo is PropertyInfo)
                {
                    ...
                    options.Add(new OptionDefinition(name, category, subCategory, sortPriority, open,
