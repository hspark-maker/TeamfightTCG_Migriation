...
namespace SRDebugger
{
    ...
    public sealed class OptionDefinition
    {
        ...
        private OptionDefinition(string name, string category, string subCategory, int sortPriority, bool isOpen)
        {
            Name = name;
            Category = category;
        ...
        public OptionDefinition(string name, string category, string subCategory, int sortPriority, bool isOpen , SRF.Helpers.MethodReference method)
            : this(name, category, subCategory, sortPriority, isOpen)
        {
            Method = method;
        }

        public OptionDefinition(string name, string category, string subCategory, int sortPriority, bool isOpen , SRF.Helpers.PropertyReference property)
            : this(name, category, subCategory, sortPriority, isOpen)
        {
            Property = property;
        }

        public static OptionDefinition FromMethod(string name, Action callback, string category = "Default", int sortPriority = 0, bool isOpen = true)
        {
            return new OptionDefinition(name, "", category, sortPriority,isOpen, callback);
        }
        ...
        public static OptionDefinition Create<T>(string name, Func<T> getter, Action<T> setter = null, string category = "Default", int sortPriority = 0, bool isOpen = true)
        {
            return new OptionDefinition(name, category, "", sortPriority, isOpen, PropertyReference.FromLambda(getter, setter));
        }
    }
}
