        ...
        protected void Populate()
        {
            ...
            foreach (var option in Service.Options.Options)
            {
                if (!OptionControlFactory.CanCreateControl(option))
                {
                    ...
                    {
                        Debug.LogWarning("[SRDebugger.OptionsTab] Unsupported property type: {0} (on property {1})".Fmt(option.Property.PropertyType, option.Property));
                    }
                    else
                    {
                        Debug.LogWarning("[SRDebugger.OptionsTab] Unsupported method signature: {0}".Fmt(option.Name));
                    }
                    continue;
                }
                ...
                var tag = option.SubCategory != string.Empty ? option.SubCategory : option.Category;
                if (!sortedOptions.TryGetValue(tag, out memberList))
                {
                    memberList = new List<OptionDefinition>();
                    sortedOptions.Add(tag, memberList);
                }
            ...
            {
                if (kv.Value.Count == 0 || 
                    kv.Value.FindIndex(o => o.IsOpen) == -1)
                {
                    continue;
                }
